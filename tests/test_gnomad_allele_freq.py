"""
Unit tests for the gnomad_allele_freq module.
These tests validate:
- Allele frequency formatting logic
- Robust handling of gnomAD API responses and failures
- Correct aggregation of genome and exome allele frequencies
- End-to-end extraction of allele frequency summaries from VCF input
All external dependencies (gnomAD API calls and VCF parsing) are mocked so that:
- No network requests are made
- No real VCF files are required
- Test failures indicate logic errors within this module only
"""
...
@patch(
    "clinvar_anno_core.modules.gnomad_allele_freq.requests.post",
    side_effect=Exception("fail"),
)
def test_query_gnomad_failure(_mock_post):
    """
...
def test_extract_afs_success(mock_query_gnomad, mock_parse_vcf):
    ...
    """
    # Simulate the VCF parser returning two variant IDs
    mock_parse_vcf.return_value = [
        "1:100:A:T",
        "1:200:G:C",
    ]
